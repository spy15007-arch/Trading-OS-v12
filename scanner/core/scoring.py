...
def lorentzian_distance(
    ...
    d1 = math.log(1 + abs(current_rsi - ideal_rsi))
    d2 = math.log(1 + abs(current_rvol - ideal_rvol))
...
def score_stock(df):
    ...
    vwap_now = float(vwap(df).iloc[-1])

    rel_vol = float(relative_volume(df))

    candle_strength = closing_strength(df)
